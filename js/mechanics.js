...
function tjDeltaText(v){ return v>0?`<b class="up">+${v}</b>`:v<0?`<b class="dn">${v}</b>`:'<b>0</b>'; }

function tjRepeatDamage(){
  if(S.tjCount===2){
...
function tjBigInjury(cont){
  ...
  tjRepeatDamage(); afterGamble('fail',cont);
}
...
function pitcherContractCap(){ return ({SP:7,CL:5,MR:4})[S.role]||7; }
...
function simSeason(lv){
  ...
  if(S.pos==='P'){
    ...
    const q=a.con*0.5+a.pow*0.2+a.eye*0.18+a.spd*0.12, d=q-par-0.5; st.d=d; 
    let staF;
    ...
    if(d>=10 && staF<0.75 && S.dpos!=='DH' && S.dpos!=='C'){ staF=Math.max(staF,0.9); dhThisYear=true; }
...
function defRuns(lv,overrideDp,games){
  if(S.pos==='P')return 0;
...
function pitcherSalaryRole(st,recordedRole){
...
function currentSalaryRating(fallback){
...
function normalizeBatterStats(st,lv){
...
function normalizePitchingStats(st,lv){
  const maxG=LV[lv].g||0;
  st.G=clamp(Math.round(st.G||0),0,maxG);
...
function accStat(bucket,st){
  ...
  t.DEF+=(st.DEF||0); t.IP=+(t.IP+st.IP).toFixed(1);
}
...
function hasMlbService(){ return !!(S&&((S.stats&&S.stats.MLB&&S.stats.MLB.yr>0)||(S.log||[]).some(r=>r.lv==='MLB'))); }
function levelMinAnnual(lv){ return /^R$|^A[123]$/.test(lv)&&hasMlbService()?MLB_SERVICE_MINOR_MIN:(LEVEL_MIN_ANNUAL[lv]||0); }
...
function calcContractAnnual(lv,d,mult){ return Math.round(Math.max(levelMinAnnual(lv),salaryFor(lv,d)*(mult||1))); }
function ratingAtLevel(d,fromLv,toLv){ const value=Number.isFinite(d)?d:0,from=LV[fromLv],to=LV[toLv]; return from&&to?+(value+from.par-to.par).toFixed(2):value; }
function contractAnnual(){
  ...
  if(S.ct)S.ct.annual=annual; 
  return annual;
}
function makeContract(yrs,mult,lv,d,annual,extra){
...
function postingReleaseFee(guaranteed){
...
function controlledAnnual(lv,d,healthMult){
  const market=salaryFor(lv,d), svc=clamp(S.svc||1,1,4);
  const cfg={ CPBL1:{rates:[0,0.70,0.78,0.86,0.94]}, NPB1:{rates:[0,0.70,0.80,0.90,0.96]}, MLB:{rates:[0,0.08,0.12,0.35,0.60]} }[lv];
...
function dposReview(cont){
  ...
  if(S.pos==='C'){ 
    ...
    if(S.dpos==='C'){
      if(cOk()){ cont(); return; }
      const opts=[];
      if(dpQual('1B'))opts.push({t:'移防 一壘手',main:true,s:posAdjLabel('1B'),f:()=>{S.dpos='1B';card('info','守位調整','捕手裝備收進置物櫃——新球季改守<b class="hl">一壘</b>。');cont();}});
      opts.push({t:'轉任 指定打擊',main:!opts.length,s:posAdjLabel('DH'),f:()=>{S.dpos='DH';card('info','守位調整','阻殺率成了聯盟笑話，球團決定讓你專心打擊——<b class="hl">DH</b>。');cont();}});
      choose(`守位會議：教練團已經不敢讓你蹲捕（${LV[S.lv].n}標準）`,opts); return;
    }
    if(cOk()){ 
      choose('守位會議：牛棚捕手回報你的接捕又行了',[
        {t:'重披捕手裝備',main:true,s:posAdjLabel('C'),f:()=>{S.dpos='C';card('good','守位調整','面罩戴回來——新球季重新登錄為<b class="hl">捕手</b>。');cont();}},
  ...
  if(dpQual(S.dpos)){
    ...
    if(DP_RANK[best]<DP_RANK[S.dpos]){ 
      choose(`守位會議：教練團想把你推上更吃重的位置`,[
        {t:`升防 ${DPN[best]}`,main:true,s:posAdjLabel(best),f:()=>{S.dpos=best;card('good','守位調整',`守備數據說服了所有人——新球季改守 <b class="hl">${DPN[best]}</b>。`);cont();}},
        {t:`留守 ${DPN[S.dpos]}`,f:()=>cont()}]); return; }
    cont(); return; }
  const opts=q.slice(0,2).map((p,i)=>({t:`移防 ${DPN[p]}`,main:i===0, s:p==='DH'?`守備已無處可站｜${posAdjLabel(p)}`:posAdjLabel(p), f:()=>{ S.dpos=p; card('info','守位調整',`球團季末評估後，新球季改守 <b class="hl">${DPN[p]}</b>。`); cont(); }}));
  choose(`守位會議：教練團認為你的守備已撐不住 ${DPN[S.dpos]}（${LV[S.lv].n}標準）`,opts);
}

function amateurSeason(){
  if(S.seasonFactor===0){ card('bad','','整季只能在場邊看著隊友比賽。'); S.log.push({y:S.year,age:S.age,tm:S.team||typeof stageLabel === 'function' ? stageLabel() : '',line:'傷缺全季', inj:true}); nextStep(); return; }
  ...
  S.pool+=gain;
  S.log.push({y:S.year,age:S.age,tm:S.team||typeof stageLabel === 'function' ? stageLabel() : '',line:plain.join('、'), inj:false});
...
function proSeason(){
 const seasonLv=S.lv,st=simSeason(seasonLv); S.lastSt=st; S.lastD=st.d; S.lastLv=seasonLv;
  ...
  else{ normalizeBatterStats(st,seasonLv); st.DEF=defRuns(seasonLv,st._dh?'DH':null,st.G); }
  S.lastD=st.d;
  ...
  else card('','球季數據',`<span class="tag">${typeof S.teamName === 'function' ? S.teamName() : S.orgTeam}${S.dpos?'｜'+S.dpos:''}</span><div class="statline">${statLine(st)}</div>`);
  if(st.form===-1){
  ...
  S.log.push({y:S.year,age:S.age,tm:typeof S.teamName === 'function' ? S.teamName() : S.orgTeam,lv:seasonLv,p:S.dpos||'',role:S.pos==='P'?S.role:null,line:S.seasonFactor===0?'傷缺全季':statLine(st), inj: isInj, st: st});
  ...
  if(S.pos!=='P'){ const tg=toolGap();
    ...
    if(!S.traits.onetool && !isRegular && tg.gap>=22 && tg.val>=58 && careerAllStars()<4){ S.traits.onetool=true;
      ...
      if(wasBefore||S.age>=33) traitCard('onetool','只會這個',`歲月帶走了你的其他工具，只剩<b class="hl">${role}</b>那一項本領還在。教練把你當成板凳上的秘密武器——關鍵時刻，你仍然可靠。`,'bad');
      else traitCard('onetool','只會這個',`你只有一項武器強得誇張，其餘全是破洞。教練不敢讓你先發，只在關鍵時刻派你上去做一件事——你成了球隊的<b class="hl">${role}</b>。出賽數銳減，但那一項本領無人能及。`,'bad'); }
...
function awardP(value,hardLow,autoWin,base=25,lower=false){
  const ineligible=lower?value>hardLow:value<hardLow;
  const automatic=lower?value<=autoWin:value>=autoWin;
  if(ineligible)return 0; if(automatic)return 100;
...
function awards(bucket,st){
  ...
  { const d=st.d;
    let asP=awardP(d,-2,10,15);
    if(asP>0&&asP<100&&bucket==='CPBL'&&S.orgTeam==='台中猛獁')asP=clamp(asP+30,15,97); 
    if(chance(asP)){ S.stats[bucket].AS++; h.push(`${y} ${lgN}明星賽`+((bucket==='CPBL'&&S.orgTeam==='台中猛獁'&&d<2)?'（人氣入選）':'')); } }

  const rookieOK=bucket!=='CPBL'||!(S.stats.NPB||S.stats.MLB||S.stats.MINOR);
  if(S.stats[bucket].yr===1&&rookieOK){
    const rkP=awardP(st.d,4,10,30); if(chance(rkP)) h.push(`${y} ${lgN}新人王`);
  }
  ...
  if(S.pos==='P'){
    ...
    if(S.role==='CL'){ const p=awardP(st.SV,th.sv[0],th.sv[1],28); if(chance(p)) h.push(`${y} ${lgN}救援王`); }
    if(S.role==='MR'){ const p=awardP(st.HLD||0,th.hld[0],th.hld[1],28); if(chance(p)) h.push(`${y} ${lgN}中繼王`); }
    { const p=awardP(st.SO,th.so[0],th.so[1]); if(chance(p))h.push(`${y} ${lgN}三振王`); }
  }
  else{
    if(st.PA >= 350){ const p=awardP(st.avg,th.avg[0],th.avg[1]); if(chance(p)) h.push(`${y} ${lgN}打擊王`); }
    if(st.PA >= 300){ const p=awardP(st.HR,th.hr[0],th.hr[1]); if(chance(p)) h.push(`${y} ${lgN}全壘打王`); }
    if(st.PA >= 300){ const p=awardP(st.SB,25,45); if(chance(p)) h.push(`${y} ${lgN}盜壘王`); }
    if(st.PA >= 300){ const p=awardP(st.RBI,th.rbi[0],th.rbi[1]); if(chance(p)) h.push(`${y} ${lgN}打點王`); }
    const obp = st.PA > 0 ? (st.H + st.BB) / st.PA : 0;
    if(st.PA >= 350){ const p=awardP(obp,th.obp[0],th.obp[1]); if(chance(p)) h.push(`${y} ${lgN}上壘王`); }
    const def1 = st.DEF || 0; const awardDp=st._dh?'DH':(S.dpos||(S.pos==='C'?'C':null));
    const gloveMinG=Math.ceil(LV[S.lv].g*0.5);
    if(awardDp&&awardDp!=='DH'&&st.G>=gloveMinG){
      const gt=GLOVE_TH[awardDp]||[4,16]; const pGlove=awardP(def1,gt[0],gt[1],30); if(chance(pGlove))h.push(`${y} ${lgN}${DPN[awardDp]}金手套`);
      const pBible=awardP(def1,9,22,25); if(chance(pBible))h.push(`${y} ${lgN}守備聖經`);
    }
  }
  ...
  if(mvpQual&&S.seasonFactor>=0.9){
    if(isReliever){
      if(st.d>=10&&st.era<=2.20){
        const pMVP=clamp(0.5+(st.d-10)*0.6+(2.20-st.era)*2+Math.max(0,(st.SV||0)-35)*0.08,0.5,6);
        if(chance(pMVP))h.push(`${y} ${lgN}年度MVP`);
      }
    }else{
      const pMVP=awardP(st.d,6,15,12); if(chance(pMVP))h.push(`${y} ${lgN}年度MVP`);
    }
  }
  ...
  if(added.length){ card('gold','年度獎項',added.map(x=>x.slice(5)).join('｜'));
    ...
    if(S.traits.glass&&!S.traits.phoenix){ const big=added.some(x=>/MVP|最佳投手|打擊王|全壘打王|新人王/.test(x));
      if(big){ S.traits.phoenix=true; removeTrait('glass','玻璃人'); S.pool+=8;
        card('gold','隱藏屬性解鎖：浴火重生','那些殺不死你的，真的讓你更強大了。撕裂的韌帶長成更堅韌的形狀——<b class="hl">玻璃人懲罰解除，受傷率恢復正常，並獲得一大筆能力點</b>。'); } }
  }
}

function intlStatLine(st){
  if(S.pos==='P'){ const era=st.IP>0?(st.ER*9/st.IP).toFixed(2):'-'; return `出賽 ${st.G}｜${fmtIP(st.IP)} 局｜${st.W} 勝｜${st.SV} 救援｜${st.SO} 三振｜ERA ${era}`; }
  const avg=st.AB>0?(st.H/st.AB).toFixed(3).replace(/^0/,''):'-'; return `出賽 ${st.G}｜${st.PA} 打席｜打擊率 ${avg}｜${st.H} 安｜${st.HR} 轟｜${st.RBI} 打點`;
}
function addIntlStat(st){ Object.keys(st).forEach(k=>{ S.intlStat[k]=(S.intlStat[k]||0)+st[k]; }); S.intlStat.IP=+S.intlStat.IP.toFixed(1); }
function intlMvpRate(st,finish){
  if(finish>1)return 0; 
  let score=0;
  if(S.pos==='P'){ const era=st.IP>0?st.ER*9/st.IP:9; score=st.IP+st.SO*1.5+st.W*8+st.SV*6+Math.max(0,3.5-era)*5-Math.max(0,era-3.5)*4; }
  else{ const avg=st.AB>0?st.H/st.AB:0; score=st.H*2+st.HR*8+st.RBI*2+Math.max(0,avg-.250)*100; }
  const finalistMult=finish===0?1:.2; return Math.round(clamp((score-28)*1.7,0,75)*finalistMult);
}
function intlFormat(wbc){
  return wbc ? {minOvr:55,par:LV.MLB.par,ranks:['冠軍','亞軍','四強止步','八強止步','預賽出局'],games:[7,7,6,5,4]}
...
function maybeIntl(done){
  ...
  if(forced){
    card('info','體育部公文',first ? `「查 台端符合國家代表隊遴選資格，依規定<b class="hl">強制徵召</b>，並自即日起<b class="hl">列管五年</b>，列管期間各國際賽事皆須配合徵召，不得以任何理由推辭。」——你甚至還沒拆完信封，行李箱已經被球團打包好了。`
    ...
    {t:forced?'⋯⋯只能報到（強制徵召）':'披上國家隊戰袍',main:true,s:'依成績獲得能力點｜下季受傷機率 +10%',f:()=>{
      ...
      { const a=S.ab, par=intlFmt.par, clutch=S.traits.clutch?1:0;
        if(S.pos==='P'){ const dd=(a.vel+a.ctl+a.brk)/3-par;
          let g, ip;
          if(isSP()){ g = teamGames>=6?ri(1,2):1; ip = +(g * (4.5 + R() * 2.5)).toFixed(1); } 
          else { g = clamp(Math.round(teamGames*(0.55+R()*0.25)),1,teamGames); ip = +(g * (0.8 + R() * 0.8)).toFixed(1); }
          const k9=clamp(7.5+dd*0.12+clutch*.5,4,14); const era=clamp(3.6-dd*0.16-clutch*.35,0.8,8);
          ...
          const g=teamGames, pa=g*ri(3,4); const ab=Math.round(pa*0.86);
