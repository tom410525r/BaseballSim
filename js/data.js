...
const POS_AB = {
    ...
    TW: ['sta','vel','ctl','brk','con','pow','spd','eye','rng','fld','arm']
};
...
function teamListOf(org) {
    return org === 'CPBL' ? CPBL_TEAMS : org === 'NPB' ? NPB_TEAMS : MLB_TEAMS;
}
...
function signTo(org, lv, team, yrs, mult) {
    ...
    if (newTeam !== S.orgTeam) {
        S.teamYears = 0;
        S.champThisTeam = false;
        S.champTeam = null;
    }
    ...
    card('info', '簽約', `與 <b class="hl">${S.teamName()}</b> 簽下 <b class="hl">${S.ct.yrs} 年</b>合約${S.ct.mult !== 1 ? `（年薪係數 ×${S.ct.mult.toFixed(2)}）` : ''}。`);
    board(2);
}
