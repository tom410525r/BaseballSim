...
function loveEvent(next) {
    ...
    if(L.st === 'dating') {
        ...
        if(chance(30)) {
            ...
            if(r < 70) { 
                const gt = loveGainTxt('sta', 1); board(1);
                card('good', '明星賽放閃', `明星賽表演賽，鏡頭掃到看台上的 <b class="hl">${L.partner}</b>，你隔著全場比了一個手勢——${gt}。`); 
                ask(); return; 
            }
            const gt = loveGainTxt('sta', 1); board(1);
        ...
        choose('記者把麥克風遞到你面前：「兩位是在交往嗎？」', [
            {t:'大方承認：「請大家祝福我們」', s:'還要看她那邊敢不敢承認', f:()=>{
                if(chance(65)) { 
                    ...
                    card('bad', '單方面承認', `她隔天透過經紀公司否認：「只是普通朋友。」據傳啦啦隊<b class="dn">禁愛令</b>壓力不小。`); 
                }
                next(); 
            }},
...
function runDraft(fromSchool, cb) {
    ...
    if(rd === 0) { card('bad', '選秀落榜', '唱名一輪又一輪，始終沒有你的名字。'); if(fromSchool) { card('info','','回到球隊。'); cb(); } else cb('fail'); return; }
